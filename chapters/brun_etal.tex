...
In confronting LD with Breton language data, we noticed difficulties: \tabref{tab:3} shows an excerpt of the data where the algorithm repeatedly failed to analyze the data, while at the same time the FuzzyMatch function did not work as we can see in \tabref{tab:3}. This last function returns a percentage reflecting the probability of correspondences (and differences) between two words.\footnote{See  https://x443.wordpress.com/2012/06/25/fuzzymatch-in-vbavbscript/}
