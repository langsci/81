...
The data from the national surveys \textsc{LAJ} and \textsc{GAJ} was compared with that from \textsc{FPJD} to trace real-time changes, while the \textsc{FPJD} data was compared with that from \textsc{CS} to examine \isi{apparent-time changes}.  The \textsc{CS} informants were 631 college students from various localities in Niigata prefecture. To construct the \textsc{CS} maps such as \figref{fig:4c}, \figref{fig:5c} and \figref{fig:6c} below, symbols were plotted at the location of each student's town of origin.
