...
\label{ex:1}
%\langinfo{}{}
\ex Cathay ... it’s ... like twelve \textstyleSubtleEmphasis{\textit{man}} \textit{(ten thousand)} or something
\end{exe}

\begin{exe}
\label{ex:2} 
%\langinfo{}{}
\ex there’re about ... ten when we had the \textstyleSubtleEmphasis{\textit{eikai-}}\textit{ (English conver- ...)} not the \textstyleSubtleEmphasis{\textit{eikaiwa}}   \textit{(English conversation)} the \textstyleSubtleEmphasis{\textit{enkai}} \textit{(party)}
\end{exe}
...
\label{ex:6}
%\langinfo{}{}
\ex there's a \textit{Monbusho}\textit{ (Ministry of Education)} scholarship isn't there
\end{exe}
