...
To define the concept of ``field worker \isi{isogloss}{\textquotedbl} (FWI) one has to start by explaining the term ``isogloss{\textquotedbl}. Isoglosses are usually characterized as boundary lines between two dialect realizations of a linguistic phenomenon (cf. \citealt[296--297]{gluck_metzler_2005}). An isogloss – or sometimes only a bundle of more than one isogloss – then forms the border of a speech area in traditional \isi{dialect geography}. The phenomenon is illustrated in Figure~\figref{fig:1}. Here, the different colors of the speech bubbles symbolize the different realizations of the linguistic variable under investigation and the blue line stands for the isogloss.
...
In this paper I will try to explain – based on the example of the Middle Franconian Speech Atlas\is{speech atlas|(} (SMF) – how field worker effects can find their way into linguistic data and analysis. While there are a number of established practices used by project members to avoid this problem, even atlases published in the last 10 years betray traces of individual field workers{\textquotesingle} influence. I will then show how dialectometry\is{dialectometry|(} can be useful to discover fieldworker phenomena in transcribed corpora of linguistic data surveyed by different people in different places.
...
As Kerswill and Wright (1990: 273) describe it: ``Transcription is a messy thing{\textquotedbl} \citep[273]{kerswill_limits_1990}. I will try to apply that statement to the specifics of the SMF on the following pages.
...
Figure~\figref{fig:3} shows a part of one page from the Middle Franconian questionnaire. The ``questions{\textquotedbl} are printed in the left column. They are sometimes real questions like in number 6 {\textquotedbl}How do you call it when a cow does not give any milk for some time before she calves?{\textquotedbl} The answer here can be translated literally as ``stands dry{\textquotedbl} and is written down in Theutonista in the right column by the field worker. This happens virtually ``on air{\textquotedbl}, while the informant is waiting for the next question. It is evident that such time pressure can lead to imprecision or even mistakes. This becomes even clearer when we have a closer look at the transcription system here.
...
Field worker isoglosses, then, are present in speech atlases\is{speech atlas|)} and are presented as {\textquotedbl}real isoglosses{\textquotedbl} separating speech variants.
...
A last point concerns the question of the degree of details in phonetic transcription\is{transcription|)}. Do we really need all those diacritics if it is seemingly impossible for two people to use them in the same way?
