...
\begin{table}
\resizebox{\textwidth}{!}{
\begin{tabular}{lp{.2\textwidth}p{.2\textwidth}p{.2\textwidth}p{.2\textwidth}p{.2\textwidth}p{.2\textwidth}} & Stressed pronoun & Verb & Simple past & Possessive & Reflexive pronoun & Object pronouns\\
\lsptoprule
...
\subsection{Grammar}
In terms of its grammatical behaviour, this phenomenon predicts the extension of the 3pl in all the syntactic elements anchoring \textit{ustedes}. Since \textit{ustedes} induces 3pl inflections, these must appear gradually, until they are settled in all the elements with reference to \textit{ustedes}. According to the map in \figref{lara-fig2}, the extension of the 3pl agreement follows the implicational hierarchy expressed in (4).
...
If the percentages are divided on the basis of the size of the population of the locality surveyed, the results are quite different, as the map in \figref{fig:4} shows.
