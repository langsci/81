...
\item[(e)] A ‘\isi{VRT-Dutch}’ cluster, which only contains standard Dutch features, such as the realisation of final \textit{t} in the words \textit{niet} and \textit{dat} (24) or the lack of expletive \textit{dat} (51).
\end{itemize}
...
On the basis of \figref{fig:2}, it is possible to suggest that the language repertoire in Ypres is of a diaglossic nature, stretching from dialect to standard language, with a range of intermediate variations. However, as argued in \sectref{sec:ditodia}, the personal repertoires of the individual speakers need to be studied first, as the overall diaglossic image might result from a combination of mainly diglossic personal repertoires, each comprising slightly different language codes.
...
\parbox{2cm}{\raggedright d) ‘Standard Dutch with an accent’} &
\parbox{2.2cm}{\raggedright e) VRT-Dutch\\}\\
\midrule
